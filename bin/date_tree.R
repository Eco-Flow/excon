...
#
# Adapted from make_dated_trees.R by Tom Wenseleers (KU Leuven, 2026), which
...
# to those data; here the same approach is driven by a user-supplied calibration
# table so it applies to any dataset. The validation it performs — treating
# chronos warnings as fatal and checking that the fitted node ages actually match
# the calibrations — is kept, as is the idea of recording calibration provenance
# alongside the tree.
#
...
# supplied by the user. This replaces chronoMPL() for anyone who has fossil or
# published calibrations: chronoMPL only makes a tree ultrametric, it does not
# put it on a time axis, so the lambda CAFE5 estimates from it is not per Myr.
#
...
# Tip labels carry a '.clean' suffix inside the pipeline; match with or without
# it so calibration tables can use plain species names.
