...
print "Running: ChopGO_VTS2.pl -i $target --GO_file $go_file -bg $background -pval $pval -pval_type $type -max_plot $go_max_plot\n";

my $exit = system("ChopGO_VTS2.pl -i $target --GO_file $go_file -bg $background -pval $pval -pval_type $type -max_plot $go_max_plot");
