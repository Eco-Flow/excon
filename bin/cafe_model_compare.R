...
parse_nll <- function(results_dir, label) {
  ...
  if (length(score_line) == 0) {
    warning(paste(
      "No 'Final Likelihood' line found in",
      files[1]
    ))
    return(NA)
  }
  val <- as.numeric(gsub(
    ".*Final Likelihood \\(-lnL\\):\\s*",
    "",
    score_line[1]
  ))
  cat(label, "NLL:", val, "\n")
...
results$LRT_vs_base_stat[results$model == "gamma"] <-
  lrt_base_vs_gamma$statistic
results$LRT_vs_base_p[results$model == "gamma"] <-
  lrt_base_vs_gamma$p_value

# --- Write outputs ---

write.table(
  results, "cafe_model_comparison.tsv", sep = "\t", quote = FALSE,
  row.names = FALSE
)
