...
### Added
- **Optional IQ-TREE2 species tree (`--iqtree_species_tree`).** Replaces OrthoFinder's own STAG/STRIDE species tree with a supermatrix maximum-likelihood tree, the more standard approach for a published species phylogeny. Three new steps run when the flag is set: `CONCAT_SINGLE_COPY` concatenates the alignments OrthoFinder already produced for every strictly single-copy, complete orthogroup into a supermatrix plus a per-orthogroup partition file (`bin/concat_single_copy.py`); `IQTREE_SPECIES_TREE` runs IQ-TREE2 with per-partition model selection (`-p -m MFP`) and 1000 ultrafast bootstrap / SH-aLRT replicates; and `ROOT_TREE` roots the result, since IQ-TREE2 returns an unrooted tree but CAFE5 requires a rooted one (`bin/root_tree.py`). The rooted tree then feeds the existing `RESCALE_TREE` → `CAFE_PREP` path unchanged. Outputs go to `results/species_tree/`.
...
- New `--iqtree_args` parameter: extra arguments appended to the IQ-TREE2 command line, e.g. `-m LG+F+G4` to override model selection or `--gcf` to add gene concordance factors.
- `--iqtree_species_tree` requires `--orthofinder_method msa` (OrthoFinder only writes `MultipleSequenceAlignments/` in MSA mode) and is rejected alongside `--input_tree`/`--input_orthogroups` or `--skip_cafe`, each with a specific error message rather than a downstream failure.
